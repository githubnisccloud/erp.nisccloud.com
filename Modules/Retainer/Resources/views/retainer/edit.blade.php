...
@push('scripts')
    ...
    <script>
        $(document).ready(function () {
            $("input[name='retainer_type_radio']:checked").trigger('change');
        });
    ...
    @if (module_is_active('Taskly') )
        <script>
            $(document).on('change', '.item', function() {
                ...
                var retainer_type =  $("#retainer_type").val();
                if (retainer_type == 'project') {
                    $("#tax_project").change();
                }
            });
    ...
    <script>
        $(document).on('change',"[name='retainer_type_radio']", function() {
            var val = $(this).val();
            $(".retainer_div").empty();
            var retainer_module = '{{$retainer->retainer_module}}';
            if(val == 'product')
            {
                ...
                $(".retainer_div").append(label);
                $("#retainer_type").val('product');

                if(retainer_module == 'account')
                {
            ...
            else if(val == 'project')
            {
                ...
                var label  = ` {{ Form::label('project', __('Project'),['class'=>'form-label']) }} {{ Form::select('project',$projects,$retainer->category_id, array('class' => 'form-control','required'=>'required')) }}`
                $(".retainer_div").append(label);
                $("#retainer_type").val('project');
                var project_id = $("#project").val();

                if(retainer_module == 'taskly')
                {
                    $("#acction_type").val('edit');
                }
                else
        ...
        function SectionGet(type = 'product',project_id = "0" ,title = 'Project'){
            ...
            $.ajax({
                type: 'post',
                url: "{{ route('retainer.section.type') }}",
                data: {
                    _token: "{{ csrf_token() }}",
                    type: type,
    ...
    @if (module_is_active('Account') && $retainer->retainer_module =='account')
        <script>
            $(document).ready(function () {
                SectionGet('product');
            });
        </script>
    @elseif (module_is_active('Taskly') && $retainer->retainer_module =='taskly')
        <script>
            $(document).ready(function () {
...
@section('content')
    <div class="row">
        {{ Form::model($retainer, array('route' => array('retainer.update', $retainer->id), 'method' => 'PUT','enctype'=>'multipart/form-data','class'=>'w-100')) }}
        @if ( $retainer->retainer_module =='account')
            <input type="hidden" name="retainer_type" id="retainer_type" value="product">
        @elseif ( $retainer->retainer_module =='taskly' )
            <input type="hidden" name="retainer_type" id="retainer_type" value="project">
        @endif
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group" id="customer-box">
                                ...
                                {{ Form::select('customer_id', $customers,!empty($retainer->user_id) ? $retainer->user_id : null, array('class' => 'form-control choices ','id'=>'customer','data-url'=>route('retainer.customer'),'required'=>'required','placeholder' =>'Select Customer')) }}
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="row">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="d-flex radio-check">
                                                <div class="form-check form-check-inline form-group col-md-3">
                                                    <input type="radio" id="product" value="product" name="retainer_type_radio" class="form-check-input code" {{ ($retainer->retainer_module =='account') ? 'checked="checked"' :''}}>
                                                    <label class="custom-control-label " for="product">{{__('Item Wise')}}</label>
                                                </div>
                                                <div class="form-check form-check-inline form-group col-md-3 ">
                                                    <input type="radio" id="project1" value="project" name="retainer_type_radio" class="form-check-input code " {{ ($retainer->retainer_module =='taskly') ? 'checked="checked"' :''}}>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group retainer_div">
                                        @if ( module_is_active('Account'))
                                        ...
                                        @elseif (module_is_active('Taskly'))
                                            ...
                                            {{ Form::select('project',$projects,$retainer->category_id, array('class' => 'form-control ','required'=>'required')) }}
                                        @endif
                                    </div>
                                </div>
                                <div class="col-md-6">
                                ...
                                @if (module_is_active('Taskly'))
                                    <div class="col-md-6 tax_project_div {{ (module_is_active('Account') ? "d-none" : '')}}">
                                        <div class="form-group">
                                            ...
                                            {{ Form::select('tax_project[]',$taxs,!empty($retainer->items->first()->tax) ? explode(",",$retainer->items->first()->tax) :null, array('class' => 'form-control get_tax multi-select choices','multiple'=>'multiple','id' => 'tax_project')) }}
                                    ...
                                    @if(module_is_active('CustomField') && !$customFields->isEmpty())
                                        <div class="col-md-12">
                                            <div class="tab-pane fade show" id="tab-2" role="tabpanel">
                                                @include('customfield::formBuilder', ['fildedata' => $retainer->customField])
                                            </div>
                                        </div>
                                    @endif
                            </div>
