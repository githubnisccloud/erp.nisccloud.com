        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var product_select = `<select class="form-control product_id item js-searchBox" name="${itemSelect}"
                                        placeholder="Select Item" data-url="{{ route('retainer.product') }}" required = 'required'>
                                        </select>`;
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.ajax({
                            ...
                            cache: false,
                            success: function(data) {
                                var retainerItems = JSON.parse(data);

                                if (retainerItems != null) {
                                    var amount = (retainerItems.price * retainerItems.quantity);

                                    $(el.parent().parent().find('.quantity')).val(retainerItems
                                        .quantity);
                                    $(el.parent().parent().find('.price')).val(retainerItems.price);
                                    $(el.parent().parent().find('.discount')).val(retainerItems
                                        .discount);
                                } else {
                                ...
                                if (retainerItems != null) {
                                    var itemTaxPrice = parseFloat((totalItemTaxRate / 100) * (
                                        retainerItems.price * retainerItems.quantity));
                                } else {
...
@if ($type == 'product')
    ...
    <div class="card repeater" @if ($acction == 'edit') data-value='{!! json_encode($retainer->items) !!}' @endif>
                        ...
                        <tr>
                            ...
                            <td width="25%" class="form-group pt-0 product_div">
                                <select name="item" class="form-control product_id item  js-searchBox"
                                    data-url="{{ route('retainer.product') }}" required>
...
@elseif ($type == 'project')
    <h5 class="pro_name h4 d-inline-block font-weight-400 mb-4 ">{{ __('Project') }}</h5>
