    ...
    <section class="padding-top padding-bottom wishlist-section">
        <div class="container">
            <div class="row product-row">
                @php
                    $count_wishlist = \Modules\LMS\Entities\Wishlist::wishCount();
                @endphp
                                        ...
                                        <div class="product-content">
                                            <div class="product-content-top">
                                                <div class="review-rating">
                                                    ...
                                                    @php
                                                        $tutor_course_count = \Modules\LMS\Entities\Course::where('workspace_id', $store->workspace_id)->where('created_by', $course->created_by)->where('status', 'Active')->get();
                                                        $tutor_id           = $tutor_course_count->pluck('created_by')->first();
                                                        $tutor_count_rating = \Modules\LMS\Entities\Ratting::where('tutor_id', $tutor_id)->where('course_id',$course->id)->where('slug', $store->slug)->where('rating_view', 'on')->count();
                                                    @endphp
                    ...
                    <div class="col-12 justify-content-center">
                        <div class="empty-detail-wrap">
                            <div class="empty-image">
                                <img alt="Image placeholder" src="{{asset('Modules/LMS/Resources/assets/img/online-shopping.svg')}}">
                            </div>
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status == "Success") {
                        show_toastr('Success', response.success, 'success');
                        if (id_2 == null) {
                            $('.fygyfg_' + id).children().attr('src', '{{asset('Modules/LMS/Resources/assets/img/wishlist.svg')}}');
                        } else {
                            $('.fygyfg_' + id_2).children().attr('src', '{{asset('Modules/LMS/Resources/assets/img/wishlist.svg')}}');
                        }
