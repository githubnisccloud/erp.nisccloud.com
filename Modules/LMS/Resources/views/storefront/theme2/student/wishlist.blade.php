@extends('lms::storefront.theme2.user.user')
@section('page-title')
...
@section('content')
    <div class="wrapper">
        <section class="common-banner-section"
            style="background-image:url({{ asset('Modules/LMS/Resources/assets/themes/theme2/images/comman-banner.png') }});">
            <div class="container">
                <div class="row">
                                            ...
                                            <div class="product-img">
                                                <a
                                                    ...
                                                    @if (!empty($course->thumbnail))
                                                        <img alt="card"
                                                            src="{{ get_file($course->thumbnail) }}"
                                                            class="img-fluid">
                                                    @else
                                                        <img src="{{ asset('Modules/LMS/Resources/assets/themes/theme2/images/bachelor-of-sci.jpg') }}"
                                                            alt="">
                                                    @endif
                                                </a>
                                                ...
                                                <div class="subtitle-top d-flex align-items-center justify-content-between">
                                                    <span
                                                        class="badge">{{ !empty($course->category) ? $course->category_name : '-' }}</span>
                                            ...
                                            <div class="product-content">
                                                <div class="product-content-top">
                                                    <div class="review-rating">
                                                        ...
                                                        @php
                                                            $tutor_course_count = \Modules\LMS\Entities\Course::where('workspace_id', $store->workspace_id)
                                                                ->where('created_by', $course->created_by)
                                                                ->where('status', 'Active')
                                                                ->get();
                                                            $tutor_id = $tutor_course_count->pluck('created_by')->first();
                                                            $tutor_count_rating = \Modules\LMS\Entities\Ratting::where('tutor_id', $tutor_id)
                                                        ...
                                                        @else
                                                            <div class="card-price">
                                                                @if ($course->has_discount == 'on')
                                                                    <ins>
                                                                        {{ $course->is_free == 'on' ? __('Free') : currency_format_with_sym($course->price,$store->created_by,$store->workspace_id) }}</ins>
                                                                @else
                                                                    <ins>
                                                                        {{ $course->is_free == 'on' ? __('Free') : currency_format_with_sym($course->price,$store->created_by,$store->workspace_id) }}</ins>
                                                                @endif
                    ...
                    if (response.status == "Success") {
                        ...
                        if (id_2 == null) {
                            $('.fygyfg_' + id).children().attr('src',
                                '{{ asset('Modules/LMS/Resources/assets/img/wishlist.svg') }}');
                        } else {
